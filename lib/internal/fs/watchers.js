...
function FSWatcher() {
  console.log('instantiating FSWatcher');
  FunctionPrototypeCall(EventEmitter, this);

  this._handle = new FSEvent();
  console.log('instantiated FSWatcher', this._handle);
  this._handle[owner_symbol] = this;

  this._handle.onedy = (status, eventType, filename) => {
    console.log('onedy', status, eventType, filename);
  };

  this._handle.onchange = (status, eventType, filename) => {
    console.log('onchange', status, eventType, filename);
...
FSWatcher.prototype[kFSWatchStart] = function(filename,
  ...
  filename = getValidatedPath(filename, 'kFSWatchStartfilename');
