...
class TestCoverage {
  ...
  summary() {
    ...
    for (let i = 0; i < coverage.length; ++i) {
      ...
      // TODO: should get from the transpiled file maybe?
  ...
  mapCoverageWithSourceMap(coverage) {
    ...
    for (let i = 0; i < result.length; ++i) {
      ...
      console.log(JSONStringify(functions, null, 2));
      ...
      console.log('source map creation', { data, lineLengths });
      ...
      for (let j = 0; j < functions.length; ++j) {
        ...
        for (let k = 0; k < ranges.length; ++k) {
          ...
          // console.log({ lines, range: ranges[k] })
          ...
          // maybe we should use "lines" here since it comes from the source map
          // comments should be ignored
          // idealmente comecar com mapped lines todas com count=1 e depois ir setando zero nos que estao em um range
          // nao coberto
          const mappedLines = this.getLines(newUrl);
          // console.log('mapped lines before', {mappedLines})
          if (!mappedLines) {
            throw new ERR_SOURCE_MAP_MISSING_SOURCE(newUrl, url);
          }

          // esses offsets deveriam ser do js nao do ts
          ...
          console.log({ mappedStartOffset, mappedEndOffset, count })

          for (let l = startEntry.originalLine; l <= endEntry.originalLine; l++) {
            console.log('setting mapped line count', { l, count })
