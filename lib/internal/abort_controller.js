...
// Background cleanup: when a composed signal is GC'd, remove dead WeakRefs
// from the source signal's kDependantSignals set. This is a safety net for
// cases where the eager purge (below) doesn't run (e.g. no further any() calls).
const dependantSignalsCleanupRegistry = new SafeFinalizationRegistry((signalWeakRef) => {
  ...
  if (signal[kDependantSignals].size === 0) {
    gcPersistentSignals.delete(signal);
  }
});

// Eagerly purge dead WeakRefs from a signal's kDependantSignals set and
// rebuild it to reclaim the Set's backing store memory. This is the primary
// cleanup mechanism in AbortSignal.any() for high-frequency usage patterns,
// because V8 WeakCells created by FinalizationRegistry.register() are
// allocated in young generation and treated as strong roots during minor GCs,
// causing them to accumulate faster than they can be collected.
// See: https://issues.chromium.org/issues/340777103
function purgeDependantSignals(signal) {
  const deps = signal[kDependantSignals];
  if (deps === undefined) return;
  const live = new SafeSet();
  deps.forEach((ref) => {
    if (ref.deref() !== undefined) live.add(ref);
  });
  signal[kDependantSignals] = live;
  if (live.size === 0) {
    gcPersistentSignals.delete(signal);
  }
}
    ...
    for (let i = 0; i < signalsArray.length; i++) {
      ...
      if (!signal[kComposite]) {
        ...
        for (const sourceSignalWeakRef of signal[kSourceSignals]) {
          ...
          purgeDependantSignals(sourceSignal);
