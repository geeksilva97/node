...
let lazyAsymmetricMatcherKey;
function asymmetricMatcherKey() {
  if (lazyAsymmetricMatcherKey === undefined) {
    lazyAsymmetricMatcherKey =
      require('internal/assert/asymmetric_matchers').kAsymmetricMatcher;
  }
  return lazyAsymmetricMatcherKey;
}

function isAsymmetricMatcher(val) {
  return val !== null &&
         typeof val === 'object' &&
         hasOwn(val, asymmetricMatcherKey());
}
...
function innerDeepEqual(val1, val2, mode, memos) {
  // Asymmetric matchers short-circuit equality. Checked before the identity
  // fast path so matchers are never compared by reference to themselves.
  if (isAsymmetricMatcher(val2)) {
    return val2.asymmetricMatch(val1);
  }
  if (isAsymmetricMatcher(val1)) {
    return val1.asymmetricMatch(val2);
  }
