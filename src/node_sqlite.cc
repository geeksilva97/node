    ...
    for (int i = 0; i < argc; ++i) {
      ...
      switch (sqlite3_value_type(value)) {
        case SQLITE_INTEGER: {
          ...
          if (std::abs(val) <= kMaxSafeJsInteger) {
  ...
  aggregate_data* GetAggregate(sqlite3_context* ctx) {
    ...
    if (!agg->initialized) {
      ...
      if (start_v->IsFunction()) {
        ...
        start_v = fn->Call(env_->context(), Null(isolate), 0, nullptr)
        ...
        /* if (!fn->Call(env_->context(), Null(isolate), 0, nullptr) */
        /*          .ToLocal(&start_v)) { */
        /*   db_->SetIgnoreNextSQLiteError(true); */
        /*   sqlite3_result_error(ctx, "", 0); */
        /*   return nullptr; */
...
void DatabaseSync::AggregateFunction(const FunctionCallbackInfo<Value>& args) {
  ...
  if (!varargs) {
    ...
    if (inverse_v->IsFunction()) {
      if (!inverse_v.As<Function>()
               ->Get(env->context(), env->length_string())
               .ToLocal(&js_len)) {
        return;
      }
  ...
  auto xInverse = inverse_v->IsFunction() ? CustomAggregate::xInverse : nullptr;
  ...
  int r = sqlite3_create_window_function(
      ...
      *name,
      ...
      new CustomAggregate(env,
                          ...
                          start_v,
                          stepFunction,
                          inverse_v->IsFunction() ? inverse_v.As<Function>()
                                                  : Local<Function>(),
                          resultFunction),
